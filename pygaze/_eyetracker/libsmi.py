...
class SMItracker(BaseEyeTracker):
	...
	def __init__(self, display, ip='127.0.0.1', sendport=4444,
		...
		**args):
		...
		res = iViewXAPI.iV_SetLogger(c_int(1), c_char_p(logfile + '_SMILOG.txt'))
		 # res = iViewXAPI.iV_SetLogger(c_int(1), os.path.join(logfile + '_SMILOG.txt'))
		if res != 1:
			err = errorstring(res)
	...
	def calibrate(self, calibrate=True, validate=True):
		...
		# moni = iViewXAPI.iV_ShowTrackingMonitor()
		# calibrate
...
##				########
		...
		if cerr == None:
			...
			if verr == None:
				...
				self.disp.fill() # clear display
				# self.screen.draw_text(text="Noise calibration: please look at the dot\n\n(press space to start)", pos=(self.dispsize[0]/2, int(self.dispsize[1]*0.2)), center=True)
				self.screen.draw_text(text="accuracy (degrees): LX=%s, LY=%s, RX=%s, RY=%s" % (self.accuracy[0][0],self.accuracy[0][1],self.accuracy[1][0],self.accuracy[1][1]), pos=(self.dispsize[0]/2, int(self.dispsize[1]*0.2)), center=True)
				self.screen.draw_text(text="accuracy (in pixels): LX=%s, LY=%s, RX=%s, RY=%s" % (self.pxaccuracy[0][0],self.pxaccuracy[0][1],self.pxaccuracy[1][0],self.pxaccuracy[1][1]), pos=(self.dispsize[0]/2, int(self.dispsize[1]*0.3)), center=True)
				self.screen.draw_text(text="accuracy (in pixels): LX=%s, LY=%s, RX=%s, RY=%s" % (self.pxaccuracy[0][0],self.pxaccuracy[0][1],self.pxaccuracy[1][0],self.pxaccuracy[1][1]), pos=(self.dispsize[0]/2, int(self.dispsize[1]*0.3)), center=True)
				self.screen.draw_text(text="precision (RMS noise in pixels): X=%s, Y=%s" % (self.pxdsttresh[0],self.pxdsttresh[1]), pos=(self.dispsize[0]/2, int(self.dispsize[1]*0.4)), center=True)
				self.screen.draw_fixation(fixtype='dot')
				self.disp.fill(self.screen)
				self.disp.show()
				self.screen.clear() # clear screen again
	...
	def close(self):
		...
		# TEJADA: change the outputfile name to the windows format (two \\ instead of \)
		filenameidf = re.sub(r'\\', r'\\\\', self.outputfile)
		print(filenameidf)
		# save data
		if self.recording:
			self.stop_recording
			# iViewXAPI.iV_StopRecording()
		res = iViewXAPI.iV_SaveData(c_char_p(os.path.join(filenameidf + '.idf')), c_char_p(self.description), c_char_p(self.participant), 1)
	...
	def log(self, msg):
		...
		res = iViewXAPI.iV_SendImageMessage(c_char_p(msg))
		if res != 1:
			err = errorstring(res)
	...
	def show_eyes_on_screen(self):
		iViewXAPI.iV_ShowEyeImageMonitor()
		
	def save_calibration_SIM(self, name):
		iViewXAPI.iV_SaveCalibration(c_char_p(name))
